...
import styles from "@/styles/timetablev2.module.css";
...
function TimeTable({ people, level, venueInfo }) {
	...
	const handleTimeCols = (sessionStart, sessionEnd) => {
		...
		timeTableDates.forEach((date) => {
			...
			/** random generate status [Available, Booked, Closed] */
			const status = ["Available", "Booked", "Closed"][Math.floor(Math.random() * 3)];
			...
			if (status === "Available") {
				...
				<Col 
					...
					className={`${styles.timeTableSessionCell} ${styles.availableCell}`}
				>{t("Open")}</Col>;
				// />;
			}
			else {
				col = 
					<Col 
						...
						data-end={FormatSessionEnd}
						aria-disabled 
						className={`${styles.timeTableSessionCell} ${status === "Booked" ? styles.rentedCell : styles.closedCell}`}
					>{(status === "Booked") ? t("Rented") : t("Closed")}</Col>;
					// />;
			}
	...
	return (
		<>
			...
			<Container className={styles.timetableContainer}>
				<Row>
					<Col className={styles.datePickerCell}>
